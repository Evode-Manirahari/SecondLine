...
The agent's default brain is **Nemotron-3-Super-120B** (NVIDIA open-weight) served over vLLM on AWS, and we wired up **NVIDIA Nemotron Speech-Streaming ASR**. Nemotron is also the agent-under-test in our evaluation harness. (For the recorded demo we ran Claude Sonnet for the smoothest single take — the architecture is model-agnostic, one env var swaps the brain — but Nemotron is the default provider and the model we evaluated and improved against.)
